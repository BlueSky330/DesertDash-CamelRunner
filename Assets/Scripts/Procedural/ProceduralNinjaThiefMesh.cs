...
/// <summary>
/// Procedurally generates a low-poly NinjaThief character mesh using the Unity Mesh API.
/// Follows the pattern established by ProceduralCamelMesh (AIG-173).
///
/// Produces a lean humanoid silhouette (~1.60 units tall):
///   Head (hood + white eyes), torso, arms, legs — skinned to a 10-bone skeleton.
///
/// Color specification (per NINJATHIEF_DESIGN_SPEC.md):
///   Body / outfit  #000000  pure black     — SubMesh 0
///   Eyes           #FFFFFF  pure white     — SubMesh 1
///   Belt / buckle  #C0C0C0  silver metal   — SubMesh 2
///   Shuriken       #D4AF37  gold star      — SubMesh 3
///
/// Usage:
///   Attach to a GameObject; mesh builds automatically on Awake (play mode) or
///   call Build() explicitly from an Editor script (e.g. NinjaThiefPrefabGenerator).
///
/// Coordinate convention:
///   Feet at Y=0, character faces +Z (forward). Units are Unity metres.
///
/// Geometry budget:
///   13 body boxes × 12 tris = 156
///   2 eye discs × 10 tris   =  20
///   2 belt boxes × 12 tris  =  24
///   4 shuriken bars × 12    =  48
///   Total                   ≈ 248 tris  (well within the &lt;1 200 target)
///
...
[ExecuteAlways]
public class ProceduralNinjaThiefMesh : MonoBehaviour
{
    // ── Color constants ────────────────────────────────────────────────────────

    /// <summary>#000000 — pure black outfit.</summary>
    public static readonly Color ColorBlack  = new Color(0f,     0f,     0f,     1f);
    /// <summary>#FFFFFF — pure white eyes.</summary>
    public static readonly Color ColorWhite  = new Color(1f,     1f,     1f,     1f);
    /// <summary>#C0C0C0 — silver metallic belt.</summary>
    public static readonly Color ColorSilver = new Color(0.753f, 0.753f, 0.753f, 1f);
    /// <summary>#D4AF37 — gold shuriken.</summary>
    public static readonly Color ColorGold   = new Color(0.831f, 0.686f, 0.216f, 1f);

    // ── SubMesh indices ────────────────────────────────────────────────────────

    private const int SM_BLACK  = 0;
    private const int SM_WHITE  = 1;
    private const int SM_SILVER = 2;
    private const int SM_GOLD   = 3;
    ...
    void Awake()
    {
        // Auto-build at runtime; Editor builds are driven by NinjaThiefPrefabGenerator.
#if UNITY_EDITOR
    ...
    /// <summary>
    /// Generate (or regenerate) the NinjaThief mesh, 10-bone skeleton, and materials.
    ...
    private Mesh GenerateMesh(Transform[] bones)
    {
        ...
        Box(new Vector3(0f, 0.75f, 0f), new Vector3(0.28f, 0.30f, 0.18f), BONE_PELVIS, SM_BLACK);

        // ── Upper torso / chest (Y: 0.90 → 1.18) ────────────────────────────
        // Slightly wider than hips to suggest athletic shoulders.
        Box(new Vector3(0f, 1.04f, 0f), new Vector3(0.34f, 0.28f, 0.20f), BONE_CHEST, SM_BLACK);

        // ── Head with hood (Y: 1.28 → 1.56) ─────────────────────────────────
        // Slightly oversized for cartoon proportions; hood integrates into the block.
        Box(new Vector3(0f, 1.42f, 0f), new Vector3(0.28f, 0.28f, 0.24f), BONE_HEAD, SM_BLACK);

        // ── Left arm ─────────────────────────────────────────────────────────
        Box(new Vector3(-0.27f, 1.04f, 0f), new Vector3(0.10f, 0.25f, 0.10f), BONE_L_ARM, SM_BLACK); // upper
        Box(new Vector3(-0.27f, 0.75f, 0f), new Vector3(0.09f, 0.28f, 0.09f), BONE_L_ARM, SM_BLACK); // forearm+hand

        // ── Right arm ────────────────────────────────────────────────────────
        Box(new Vector3( 0.27f, 1.04f, 0f), new Vector3(0.10f, 0.25f, 0.10f), BONE_R_ARM, SM_BLACK);
        Box(new Vector3( 0.27f, 0.75f, 0f), new Vector3(0.09f, 0.28f, 0.09f), BONE_R_ARM, SM_BLACK);

        // ── Left leg ─────────────────────────────────────────────────────────
        Box(new Vector3(-0.09f, 0.45f, 0f),     new Vector3(0.12f, 0.30f, 0.12f), BONE_L_LEG, SM_BLACK); // thigh
        Box(new Vector3(-0.09f, 0.19f, 0f),     new Vector3(0.10f, 0.24f, 0.10f), BONE_L_LEG, SM_BLACK); // shin
        Box(new Vector3(-0.09f, 0.05f, 0.04f),  new Vector3(0.10f, 0.10f, 0.20f), BONE_L_LEG, SM_BLACK); // boot

        // ── Right leg ────────────────────────────────────────────────────────
        Box(new Vector3( 0.09f, 0.45f, 0f),    new Vector3(0.12f, 0.30f, 0.12f), BONE_R_LEG, SM_BLACK);
        Box(new Vector3( 0.09f, 0.19f, 0f),    new Vector3(0.10f, 0.24f, 0.10f), BONE_R_LEG, SM_BLACK);
        Box(new Vector3( 0.09f, 0.05f, 0.04f), new Vector3(0.10f, 0.10f, 0.20f), BONE_R_LEG, SM_BLACK);

        // ── Eyes — white discs on front face of head (Z ≈ +0.15 proud) ───────
        // Head front face sits at Z = 0 + 0.12; discs at Z = 0.15 avoid z-fighting.
        Disk(new Vector3(-0.07f, 1.46f, 0.15f), 0.055f, 10, BONE_HEAD, SM_WHITE);
        Disk(new Vector3( 0.07f, 1.46f, 0.15f), 0.055f, 10, BONE_HEAD, SM_WHITE);

        // ── Belt — silver band at waist junction ──────────────────────────────
        Box(new Vector3(0f, 0.905f, 0f),    new Vector3(0.30f, 0.075f, 0.20f), BONE_PELVIS, SM_SILVER);
        // Belt buckle: small raised rectangle on chest front.
        Box(new Vector3(0f, 0.905f, 0.11f), new Vector3(0.09f, 0.065f, 0.02f), BONE_PELVIS, SM_SILVER);

        // ── Shuriken — gold 8-point throwing star on chest ────────────────────
        // Four thin rectangular blades at 0° / 90° / 45° / −45° form an 8-pointed star.
        // Chest front face at Z ≈ 0.10; star sits at Z = 0.115 (slightly proud).
        var starCentre = new Vector3(0f, 1.04f, 0.115f);
        var bladeSize  = new Vector3(0.26f, 0.04f, 0.015f);
        Box(starCentre, bladeSize, BONE_CHEST, SM_GOLD);                                   // horizontal
        Box(starCentre, bladeSize, BONE_CHEST, SM_GOLD, Quaternion.Euler(0f, 0f,  90f));   // vertical
        Box(starCentre, bladeSize, BONE_CHEST, SM_GOLD, Quaternion.Euler(0f, 0f,  45f));   // diagonal /
        Box(starCentre, bladeSize, BONE_CHEST, SM_GOLD, Quaternion.Euler(0f, 0f, -45f));   // diagonal \

        // ── Assemble mesh ─────────────────────────────────────────────────────
        var mesh = new Mesh { name = "ProceduralNinjaThief" };
    ...
    {
        if (matBlack  == null) matBlack  = CreateMat(ColorBlack,  0f,   0f);    // matte
        if (matWhite  == null) matWhite  = CreateMat(ColorWhite,  0f,   0f);    // matte
        if (matSilver == null) matSilver = CreateMat(ColorSilver, 0.5f, 0.4f);  // semi-metallic
        if (matGold   == null) matGold   = CreateMat(ColorGold,   0.6f, 0.5f);  // metallic sheen
        return new[] { matBlack, matWhite, matSilver, matGold };
    }
